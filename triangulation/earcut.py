...
def earcut(data, holeIndices=None, dim=2):
    ...
    hasHoles = isinstance(holeIndices, list) and len(holeIndices) > 0
...
def earcutLinked(ear, triangles, dim, minX, minY, invSize, passs=None):
    ...
    if (not passs and invSize): indexCurve(ear, minX, minY, invSize);
...
def eliminateHoles(data, holeIndices, outerNode, dim):
    ...
    leng = len(holeIndices)

    for i in range(leng):
        start = holeIndices[i] * dim
        end = holeIndices[i + 1] * dim if i < leng - 1 else len(data)
    ...
    queue = sorted(queue, key=lambda x: cmp_to_key(compareX))
...
def indexCurve(start, minX, minY, invSize):
    p = start
    while True:
        if (p.z == None): p.z = zOrder(p.x, p.y, minX, minY, invSize);
        p.prevZ = p.prev
        p.nextZ = p.next
        p = p.next
        if (p == start): break
...
def deviation(data, holeIndices, dim, triangles):
    ...
    if (hasHoles):
        lenn = len(holeIndices)
        for i in range(lenn):
            start = holeIndices[i] * dim
            end = holeIndices[i + 1] * dim if i < lenn - 1 else len(data)
...
def flatten(data):
    ...
    for i in range(len(data)):
        for j in range(len(data[i])):
            for d in range(dim): result['vertices'].append(data[i][j][d]);
...
if __name__ == '__main__':
    print(earcut([1.1123121, 2,1,2,4,3,6,5,2]))